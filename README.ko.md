...
YouTube, Pinterest, Alibaba의 프로덕션 시스템에서 영감받은 **3단계 접근법**을 사용합니다:
...
대부분의 추천 시스템 튜토리얼은 **한 조각**만 보여줍니다 — Jupyter 노트북의 협업 필터링 모델이나 하드코딩된 아이템을 반환하는 REST API. 이 프로젝트는 **전체 기계**입니다:
...
CDN (Tier 0)은 이미지나 JS 번들이 아닌 **API 응답**을 캐싱합니다. `/api/v1/popular` 엔드포인트는 모든 유저에게 동일한 JSON을 반환하며 캐시 헤더를 포함합니다:
...
시스템은 설계상 **최종 일관성(eventually consistent)**입니다. 강한 일관성을 보장하는 컴포넌트는 없습니다:
